...
Sì, Trident può essere installato offline. Fare riferimento a link:trident-install/kubernetes-deploy.html["Scopri di più sull'installazione Trident"^].
...
Il programma di installazione di Trident crea un Kubernetes ClusterRole, che ha accesso specifico alle risorse PersistentVolume, PersistentVolumeClaim, StorageClass e Secret del cluster Kubernetes. Fare riferimento a link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personalizza l'installazione di tridentctl"].



=== Posso generare localmente gli esatti file manifest utilizzati Trident per l'installazione?

È possibile generare e modificare localmente i file manifest esatti utilizzati da Trident per l'installazione, se necessario. Fare riferimento a link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personalizza l'installazione di tridentctl"].
