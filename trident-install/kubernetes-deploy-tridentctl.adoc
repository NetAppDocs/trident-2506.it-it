...
NOTE: Se il programma di installazione non viene completato correttamente o `trident-controller-<generated id>` (`trident-csi-<generated id>` nelle versioni precedenti alla 23.01) non presenta lo stato *Running*, la piattaforma non è stata installata. Usa  `-d` per link:../troubleshooting.html#unsuccessful-trident-deployment-using-tridentctl["attiva la modalità debug"] e risolvere il problema.
...
[%collapsible]
====
Per ulteriori informazioni sul distacco forzato, fai riferimento a link:kubernetes-customize-deploy.html["Personalizza l'installazione dell'operatore Trident"].
