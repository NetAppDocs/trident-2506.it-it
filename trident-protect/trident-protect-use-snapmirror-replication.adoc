...
summary: 'Utilizzando Trident Protect, è possibile sfruttare le funzionalità di replica asincrona della tecnologia NetApp SnapMirror per replicare le modifiche dei dati e delle applicazioni da un backend di storage all"altro, sullo stesso cluster o tra cluster diversi.' 
---
...
Per saperne di più sugli hook di esecuzione, fare riferimento alink:../trident-protect/trident-protect-use-execution-hooks.html["Gestire gli hook di esecuzione Trident Protect"] .
